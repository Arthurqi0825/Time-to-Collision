...
public:
    ...
    void targetSensorCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
        if (!target_initialized_) {
            ...
            target_state_prev_.center_position = Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
        ...
        target_state_curr_.center_position = Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
        computeTTC();
    }
    
    Eigen::Vector3d CalculatingVelocity(const State& previous_state, const State& current_state) {
        double dt = (current_state.timestamp - previous_state.timestamp);
        Eigen::Vector3d velocity(0.0, 0.0, 0.0);
        if (dt > 0.001) {
            velocity(0) = (current_state.center_position(0) - previous_state.center_position(0)) / dt;
            velocity(1) = (current_state.center_position(1) - previous_state.center_position(1)) / dt;
            velocity(2) = (current_state.center_position(2) - previous_state.center_position(2)) / dt;
        }
    ...
    void computeTTC() {
        ...
        if (ego_state_curr_.timestamp - ego_state_prev_.timestamp > 2 || target_state_curr_.timestamp - target_state_prev_.timestamp > 2) {
        ...
        //Sensor position to Center position
        ...
        Eigen::Vector3d relative_dist = target_center_pos_ - ego_center_pos_;
        // Collision Distance 
        Eigen::Vector3d collision_distance_vec = Eigen::Vector3d(
        ...
        ego_state_prev_ = ego_state_curr_;
        target_state_prev_ = target_state_curr_;
    }
};
